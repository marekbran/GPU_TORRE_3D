...
def surface_integral(u_data, du_dt_data, p_1_data, p_2_data, p_3_data, t_data, t_shift, source_points, orbit_nodes, orbit_triangles, device):
    ...
    scaling_vec_2 = torch.zeros((n_triangles, n_source_points)).to(device)
    ...
    n_vec_aux = torch.cross(aux_vec_1, aux_vec_2, dim=-1)
    ...
    for i in range(0, n_source_points):
        ...
        row_indices = (i * torch.ones(n_triangles, dtype=torch.int32, device=source_points.device)).to(torch.long)
    ...
    ret_time = torch.floor((s_vec-t_shift)/(d_t_2*1e5))
    print("d_t_2:", d_t_2)
    print("Min index in s_vec:", s_vec.min().item())
    print("Max index in s_vec:", s_vec.max().item())
    ...
    idx_0 = torch.zeros(num_cols, dtype=torch.int32, device=device).to(torch.long)
    idx_1 = torch.ones(num_cols, dtype=torch.int32, device=device).to(torch.long)
    idx_2 = 2 * torch.ones(num_cols, dtype=torch.int32, device=device).to(torch.long)
    ...
    integ_vec = torch.zeros((n_source_points, 2 * t_data.shape[0]), device=device)
    ...
    du_dn_data = du_dn_data.to(device)

    for t_ind in range(0,u_data.shape[1]):
        print("Iteration:", t_ind)
        print("The shape of du_dn_data", du_dn_data.shape)
        ...
        print("Min index in row_indices:", row_indices.min().item())
        print("Min index in col_indices:", col_indices.min().item())
        ...
        print("Outrput_shape:", output_shape[1])
        ...
        print("Shape of flat_aux_vec:", flat_aux_vec.shape)
        print("Shape of flat_indices:", flat_indices.shape)
        print("Shape of values_to_accumulate:", values_to_accumulate.shape)
        print("Max index in flat_indices:", flat_indices.max().item())
        print("Min index in flat_indices:", flat_indices.min().item())
        print("Size of flat_aux_vec:", flat_aux_vec.size(0))
...
if __name__ == "__main__":
    mat_data_full = scipy.io.loadmat('/media/datadisk/adam/forward_scripts/surface_integral.mat')
    mat_data = scipy.io.loadmat(r"/media/datadisk/adam/forward_scripts/bounndary_point_source.mat")
    ...
    orbit_triangles = orbit_triangles.astype(np.int32)
    ...
    test = test.cpu().numpy()
    surface_integral = surface_integral.cpu().numpy()
