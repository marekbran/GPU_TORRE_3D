...
if __name__ == "__main__":
    ...
    del pulse_window, d_pulse_window, time_arg, boundary_vec_1, boundary_vec_2
    torch.cuda.empty_cache()
    gc.collect()
    ...
    div_vec = torch.zeros((t_array.shape[0], 1), dtype=torch.float32, device=device)
     # u.dtype is used to match the original tensor's type
    ...
    p_1 = torch.zeros((t_array.shape[0], 3), dtype=torch.float32, device=device)
    p_2 = torch.zeros((t_array.shape[0], 3), dtype=torch.float32, device=device)
    p_3 = torch.zeros((t_array.shape[0], 3), dtype=torch.float32, device=device)
    ...
    del f_aux, a_o, orbit_ind
    torch.cuda.empty_cache()
    gc.collect()
    print(f"GPU memory used: {torch.cuda.memory_allocated() / (1024 ** 3)} GB")
    aux_vec_init = torch.zeros(u.shape, dtype=torch.float32, device=device)
    ...
    print(aux_vec.dtype, C.dtype, M.dtype, aux_vec_init.dtype)
